...
bool dx_i2cOpen(DX_I2C_BINDING *i2c_binding)
{
    ...
    if (i2c_binding->fd < 0) {
        printf("ERROR: I2CMaster_Open: errno=%d (%s)\n", errno, strerror(errno));
    ...
    if (result != 0) {
        printf("ERROR: I2CMaster_SetBusSpeed: errno=%d (%s)\n", errno, strerror(errno));
    ...
    if (result != 0) {
        printf("ERROR: I2CMaster_SetTimeout: errno=%d (%s)\n", errno, strerror(errno));
...
bool dx_i2cClose(DX_I2C_BINDING *i2c_binding)
{
    if (i2c_binding->opened) {
        ...
        if (close(i2c_binding->fd) != 0) {
            printf("ERROR: Could not close fd %s: %s (%d).\n", i2c_binding->name, strerror(errno), errno);
            return false;
        }
    }
    return true;
